...
public class SmartQQClient implements Closeable {
	...
	private Response<String> get(ApiURL url, Object... params) {
		System.out.println("request info:" + url.buildUrl(params).toString());
	...
	private Response<String> post(ApiURL url, JSONObject r) {
		return session.post(url.getUrl()).addHeader("User-Agent", ApiURL.USER_AGENT).addHeader("Referer", url.getReferer()).addHeader("Origin", url.getOrigin()).addForm("r", r.toJSONString()).text();
	}
	...
	private static JSONObject getResponseJson(Response<String> response) {
		...
		if (retCode == null || retCode != 0) {
			if (retCode != null && retCode == 103) {
				...
				LOGGER.info(String.format("请求失败，Api返回码[%d]", retCode));
				// throw new RuntimeException(String.format("请求失败，Api返回码[%d]",
				// retCode));
			}
		}
		return json;
